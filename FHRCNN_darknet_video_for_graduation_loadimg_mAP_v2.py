...
def cvDrawBoxes(detections, img, detections_outdir,newfilename):
    ...
    for detection in detections:
        ...
        mAP_output_temp = np.array([category,confidence,xmin, ymin, xmax, ymax], dtype = float)
        mAP_output = np.append(mAP_output, mAP_output_temp, axis = 0)
    mAP_output = np.reshape(mAP_output, (len(detections),6))
    np.savetxt(os.path.join(detections_outdir,newfilename),mAP_output)
...
def YOLO():
    ...
    '''
    detections_outdir = "/home/rog640/Tom/darknet/test_img/mAP_FHRCNN_YOLOv4"
    ...
    for jpgfile in glob.glob(path):
        ...
        image = cvDrawBoxes(detections, frame_resized, detections_outdir,str(newfilename[1]))
